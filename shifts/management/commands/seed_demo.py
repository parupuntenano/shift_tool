...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        company, _ = Company.objects.get_or_create(
            code="demo", defaults={"name": "デモ株式会社"}
        )
        ...
        CompanyMembership.objects.update_or_create(
            company=company, user=admin_user, defaults={"role": "admin"}
        )

        levels = {}
        for symbol, meaning, priority, assignable in [
            ("◎", "リーダー", 1, True),
            ("○", "対応可能", 2, True),
            ("△", "訓練中", 3, True),
            ("×", "対応不可", 99, False),
        ]:
            levels[symbol], _ = SkillLevel.objects.get_or_create(
                company=company,
                symbol=symbol,
                defaults={
                    "meaning": meaning,
        ...
        for order, name in enumerate(("受付", "検品", "出荷"), 1):
            work, _ = WorkType.objects.get_or_create(
                company=company,
                name=name,
                defaults={"display_order": order, "required_staff_per_day": 1},
            )
            works.append(work)
        ...
        for index, name in enumerate(
            ...
            for work_index, work in enumerate(works):
                level = levels[
                    (
                        "◎"
                        if index == work_index + 1
                        else "○" if (index + work_index) % 2 == 0 else "△"
                    )
                ]
