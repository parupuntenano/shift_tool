...
from shifts.domain.entities import Availability, ConstraintRule, SkillRating, StaffMember, Work
...
class MonthlyShiftGeneratorTests(TestCase):
    def test_uses_only_available_assignable_staff_and_reports_shortage(self):
        ...
        result = MonthlyShiftGenerator().generate(month, staff, works, skills, availability)
    ...
    def test_respects_preferred_off_and_max_consecutive_days(self):
        ...
        availability = [Availability(1, month.replace(day=d), True, preferred_off=d == 2) for d in range(1, 6)]
        result = MonthlyShiftGenerator().generate(month, staff, works, skills, availability)
    ...
    def test_alternates_between_two_configured_works(self):
        ...
        availability = [Availability(1, month.replace(day=d), True) for d in range(1, 5)]
        rules = [ConstraintRule("work_alternation", staff_id=1, work_ids=(10, 20), is_hard=True)]
        result = MonthlyShiftGenerator().generate(month, staff, works, skills, availability, rules)
        self.assertEqual([(item.day.day, item.work_id) for item in result.assignments], [(1, 10), (2, 20), (3, 10), (4, 20)])
    ...
    def test_incompatible_staff_are_not_assigned_to_same_work(self):
        ...
        rules = [ConstraintRule("incompatible_same_work", staff_id=1, related_staff_id=2, is_hard=True)]
        result = MonthlyShiftGenerator().generate(month, staff, works, skills, availability, rules)
        day_one = [item for item in result.assignments if item.day == month]
        self.assertEqual(len(day_one), 1)
        self.assertTrue(any(item.day == month and item.work_id == 10 for item in result.warnings))
    ...
    def test_applies_work_rest_pattern(self):
        ...
        works = [Work(10, "業務", 1)]
        skills = [SkillRating(1, 10, 1, True)]
        availability = [Availability(1, month.replace(day=d), True) for d in range(1, 7)]
        rules = [ConstraintRule("work_rest_pattern", staff_id=1, text_value="2,1", is_hard=True)]
        result = MonthlyShiftGenerator().generate(month, staff, works, skills, availability, rules)
    ...
    def test_blocks_specific_work_on_selected_weekday(self):
        ...
        skills = [SkillRating(1, 10, 1, True)]
        availability = [Availability(1, month.replace(day=d), True) for d in range(1, 7)]
        rules = [ConstraintRule("forbid_works_on_weekdays", staff_id=1, work_ids=(10,), weekdays=(4,), is_hard=True)]
        result = MonthlyShiftGenerator().generate(month, staff, works, skills, availability, rules)
